(function() {
    ...
    window.importPJS = async function(program) {
        ...
        (function() {
            const regex = /\b(loadImage|getImage)\s*\(\s*(['"])(.*?)\2\s*\)/;
            const match = programString.match(regex);
            if (match && match[3]) {
                console.log("Preloading image: " + match[3]);
                assets.cacheImageURL(assets.getImageURL(match[3]));
            }
        })();

        // Find all getSound() calls to preload
        (function() {
            const regex = /\b(getSound)\s*\(\s*(['"])(.*?)\2\s*\)/;
            const match = programString.match(regex);
            if (match && match[3]) {
